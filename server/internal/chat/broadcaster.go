...
// Broadcaster sends a payload to every participant in a room on a topic.
// It is an interface so tests can stub it out instead of talking to a real
// LiveKit server.
...
func (b *LiveKitBroadcaster) Broadcast(ctx context.Context, roomID, topic string, payload []byte) error {
	t := topic
